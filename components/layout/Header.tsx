...
export default function Header({ campaignName, shootDate, onExportHTML, onExportMarkdown }: HeaderProps) {
  return (
    <>
    <style>{`
      @media (max-width: 767px) {
        .header-date, .header-badge { display: none; }
    ...
    <header style={{
      ...
      {campaignName && (
        <>
          <span style={{ color: 'var(--border-med)', fontSize: 16 }}>/</span>
          <span style={{ color: 'var(--text)', fontSize: 14, fontWeight: 500, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {campaignName}
          </span>
          {shootDate && (
      ...
      {(onExportHTML || onExportMarkdown) && (
        <div style={{ position: 'relative', marginLeft: 'auto', flexShrink: 0 }} className="export-menu">
          <button
            style={{
