...
import { Project, OpenItemGroup } from '@/lib/types'
import { SectionHeader, ChecklistItemRow, AddButton } from '@/components/ui'
...
export default function OpenItems({ project, onChange }: Props) {
  const [editing, setEditing] = useState(false)
  const groups = project.openItemGroups ?? []

  const addGroup = () => onChange({ openItemGroups: [...groups, { id: crypto.randomUUID(), title: 'New Group', items: [] }] })
  const updateGroup = (id: string, title: string) => onChange({ openItemGroups: groups.map(g => g.id === id ? { ...g, title } : g) })
  const addItem = (groupId: string) => onChange({
    openItemGroups: groups.map(g => g.id === groupId
  ...
  return (
    <section>
      <SectionHeader eyebrow="Open Items" title="Open Items" editing={editing} onToggleEdit={() => setEditing(e => !e)} />
      ...
      {groups.map(group => (
        <div key={group.id} style={{ marginBottom: 28 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            {editing ? (
              <>
                <input value={group.title} onChange={e => updateGroup(group.id, e.target.value)}
                ...
                <button onClick={() => onChange({ openItemGroups: groups.filter(g => g.id !== group.id) })}
      ...
      {groups.length === 0 && !editing && <p style={{ color: 'var(--text-3)', fontSize: 14 }}>No open items.</p>}
    </section>
  )
}
